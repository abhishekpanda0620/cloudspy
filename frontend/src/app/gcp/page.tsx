...
export default function GcpPage() {
  ...
  const mockResources: ResourceItem[] = [
    { id: "instance-1", type: "Compute Engine", region: "us-central1", status: "running", cost: 67.80 },
    { id: "bucket-data-lake", type: "Cloud Storage", region: "us-central1", status: "active", cost: 23.45 },
    { id: "cluster-prod", type: "GKE Cluster", region: "us-west1", status: "running", cost: 156.90 },
    { id: "dataset-analytics", type: "BigQuery", region: "us-central1", status: "active", cost: 89.20 },
  ];
  ...
  const handleFetch = () => {
    ...
    setTimeout(() => {
      const mockData = [
        { service: "Compute Engine", amount: 245.67, unit: "USD" },
        { service: "Cloud Storage", amount: 89.23, unit: "USD" },
        { service: "BigQuery", amount: 156.78, unit: "USD" },
        { service: "Cloud SQL", amount: 78.45, unit: "USD" },
        { service: "Cloud Functions", amount: 34.12, unit: "USD" },
      ];
      setData(mockData);
      setLoading(false);
    }, 2000);
  ...
  const previousMonthCost = totalCost * 0.92; // Mock previous month data
      ...
      <div className="w-full max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <Image src="/globe.svg" alt="GCP" width={48} height={48} />
            <div>
              <h1 className="text-3xl font-bold text-blue-700">Google Cloud Dashboard</h1>
              <p className="text-gray-600">Monitor your GCP spending and resource utilization</p>
            </div>
          </div>
        ...
        <Card className="p-6 bg-gradient-to-r from-blue-50 to-white border-l-4 border-l-blue-400">
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="project-id" className="block text-sm font-medium text-blue-700 mb-2">
                GCP Project ID *
              </label>
              <input
                id="project-id"
                ...
                className="w-full border border-blue-200 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <Card className="p-6 bg-gradient-to-br from-green-50 to-green-100 border-green-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-700">{mockResources.length}</p>
              </div>
              <span className="text-3xl">🖥️</span>
            </div>
            <div className="mt-2">
              <span className="text-xs text-green-600">Across {gcpRegions.length - 1} regions</span>
            </div>
          </Card>
          ...
          <Card className="p-6 bg-gradient-to-br from-yellow-50 to-yellow-100 border-yellow-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-yellow-600 font-medium">Top Service</p>
                <p className="text-2xl font-bold text-yellow-700">{topServices[0]?.service || 'N/A'}</p>
              </div>
              <span className="text-3xl">{getServiceIcon(topServices[0]?.service || '')}</span>
            </div>
            <div className="mt-2">
              <span className="text-xs text-yellow-600">${topServices[0]?.amount.toFixed(2) || '0.00'} this period</span>
            </div>
          </Card>
                ...
                {data.length > 0 ? (
                  ...
                  <div className="h-48 flex items-center justify-center text-gray-500">
                    <div className="text-center">
                      ...
                      <p>Configure GCP integration to view cost trends</p>
                    </div>
                  </div>
                )}
              </Card>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-blue-700">Cost Optimization</span>
                  </div>
                  <p className="text-sm text-gray-600">Consider using sustained use discounts for long-running VMs</p>
                </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xl">⚠️</span>
                    <span className="font-medium text-green-700">Usage Alert</span>
                  </div>
                  <p className="text-sm text-gray-600">BigQuery costs increased by 18% this month</p>
                </div>
                <div className="bg-yellow-50 p-4 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xl">✅</span>
                    <span className="font-medium text-yellow-700">Recommendation</span>
                  </div>
                  <p className="text-sm text-gray-600">Enable committed use discounts for predictable workloads</p>
                </div>
              </div>
            </Card>
          </TabsContent>
            ...
            <Card className="p-6">
              ...
              {loading ? (
                <div className="flex items-center justify-center h-32">
                  ...
                  <span className="text-gray-600">Loading GCP cost data...</span>
                </div>
              ) : data.length > 0 ? (
                    ...
                    <tbody>
                      {data.map((item, idx) => (
                        <tr key={idx} className="border-b hover:bg-blue-50 transition-colors">
                          ...
                          <td className="py-3 px-4 font-bold text-blue-700">${item.amount.toFixed(2)}</td>
                ...
                <div className="text-center py-12">
                  <Image src="/globe.svg" alt="GCP" width={64} height={64} className="mx-auto mb-4 opacity-50" />
                  <h3 className="text-lg font-medium text-gray-800 mb-2">No GCP Data Available</h3>
                  <p className="text-gray-600 mb-4">Configure your GCP Project ID above and click "Refresh Data" to view your cost breakdown.</p>
                </div>
              )}
            </Card>
          </TabsContent>
            ...
            <Card className="p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {mockResources
                      .filter(resource => selectedRegion === 'all' || resource.region === selectedRegion)
                      .map((resource, idx) => (
                      <tr key={idx} className="border-b hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-3 px-4 font-bold text-blue-700">${resource.cost.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {mockResources.filter(resource => selectedRegion === 'all' || resource.region === selectedRegion).length === 0 && (
